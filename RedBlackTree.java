...
public class RedBlackTree<T extends Comparable<T>> implements SortedCollectionInterface<T> {
  ...
  /**
   * This method modifies the tree in order to keep the Red-Black Tree properties
   * 
   * @param newRedNode
   */
  private void enforceRBTreePropertiesAfterInsert(Node<T> newRedNode) {
    ...
    if (newRedNode.parent.parent == null) {
      return;
    }

    // inserted to grandparent's left child's child
    if (newRedNode.parent.parent.leftChild != null && newRedNode.parent.isLeftChild()) {
      // Case 3: Parent's sibling is red
      if (!newRedNode.parent.parent.rightChild.isBlack) {
        newRedNode.parent.parent.rightChild.isBlack = true;
        newRedNode.parent.isBlack = true;
        if (newRedNode.parent.parent != root) {
          newRedNode.parent.parent.isBlack = false;
        }

        // Check if red parent has a red grandparent
        if ((newRedNode.parent != root) && !newRedNode.parent.isBlack
            && !newRedNode.parent.parent.isBlack) {
          enforceRBTreePropertiesAfterInsert(newRedNode.parent);
        }
      }

      // Case 1: Parent's sibling black & opposite side
      else if (newRedNode.parent.parent.rightChild.isBlack && newRedNode.isLeftChild()) {
        rotate(newRedNode.parent, newRedNode.parent.parent);
        if (newRedNode.parent != root) {
          newRedNode.parent.isBlack = true;
        }
        newRedNode.parent.rightChild.isBlack = false;
      ...
      // Case 2: Parent's sibling black & same side
      else if (newRedNode.parent.parent.rightChild.isBlack && !newRedNode.isLeftChild()) {
    ...
    // inserted to grandparent's right child's child
    else if (newRedNode.parent.parent.rightChild != null && !newRedNode.parent.isLeftChild()) {
      // Case 3: Parent's sibling is red
      if (!newRedNode.parent.parent.leftChild.isBlack) {
      ...
      // Case 1: Parent's sibling black & opposite side
      else if (newRedNode.parent.parent.leftChild.isBlack && !newRedNode.isLeftChild()) {
        rotate(newRedNode.parent, newRedNode.parent.parent);
        if (newRedNode.parent != root) {
          newRedNode.parent.isBlack = true;
        }
        newRedNode.parent.leftChild.isBlack = false;
  ...
  private void rotate(Node<T> child, Node<T> parent) throws IllegalArgumentException {
    if (parent.leftChild != null && parent.rightChild != null) {
      if (parent.leftChild.equals(child.data) || parent.rightChild.equals(child.data)) {
    ...
    if (parent.parent == null) {
      // right rotation
      if (parent.leftChild != null && parent.leftChild.data.equals(child.data)) {

        // If child has right child, set parent's left child to that right child
        ...
        // 1. Set child's right child to parent
        child.rightChild = parent;

        // 2. Set parent's parent to child
        parent.parent = child;

        // 3. Set child's parent to null
        child.parent = null;

      }
      // left rotation
      if (parent.rightChild != null && parent.rightChild.data.equals(child.data)) {

        // If child has left child, set parent's right child to that left child
        ...
        // 1. Set child's left child to parent
    ...
    // Parent is not the root
      ...
      if (parent.leftChild != null && parent.leftChild.equals(child)) {
        // 1. Set child's right child to parent's left child if exists
        if (child.rightChild != null) {
          parent.leftChild = child.rightChild;
          child.rightChild.parent = parent;
        } else {
        ...
        // 2. Set parent's parent's child to either left or right
        if (parent.parent.leftChild != null && parent.parent.leftChild.data.equals(parent.data)) {
          parent.parent.leftChild = child;
        } else if (parent.parent.rightChild != null
            && parent.parent.rightChild.data.equals(parent.data)) {
          parent.parent.rightChild = child;
        }

        // 3. Set parent's parent to child
        parent.parent = child;

        // 4. Set child's right child to parent
        child.rightChild = parent;
      }
      // Left rotation
      else if (parent.rightChild != null && parent.rightChild.equals(child)) {
        // 1. Set child's left child to parent's right if exists
        if (child.leftChild != null) {
          parent.rightChild = child.leftChild;
          child.leftChild.parent = parent;
        } else {
